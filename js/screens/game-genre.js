...
import {gameArtistScreenEl} from './game-artist.js';
import {welcomeScreenEl} from './welcome.js';

const gameGenreScreenTmp = `<header class="game__header">
...
export const gameGenreScreenEl = renderScreen(gameGenreScreenTmp, `game game--genre`);

const verifyBtns = (elem) => {
  return elem.checked;
};

const gameForm = gameGenreScreenEl.querySelector(`.game__tracks`);
...
gameSubmitBtn.addEventListener(`click`, (evt) => {
  evt.preventDefault();
  updateScreen(gameArtistScreenEl);
});

const gameBackBtn = gameGenreScreenEl.querySelector(`.game__back`);
